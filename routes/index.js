...
exports.index_ntlm = function(req, res) {
    console.log(req.ntlm) // { target: 'MYDOMAIN', userid: 'MYUSERID', workstation: 'MYWORKSTATION' }
	logInInfo = req.ntlm;
	/*res.writeHead(302, {
	  'Location': '/'
...
exports.surveylist = function(req, res) {
	// Query Mongo for surveys
	//console.log('surveylist');
	console.log('In surveylist: session user ='+req.session.authUser.CUID);
	console.log('In surveylist: session isAuthSuccess='+req.session.isAuthSuccess);
...
exports.vote = function(socket) {
	socket.on('send:vote', function(data) {
		...
		var polls;
		var surveyResults = [];
		console.log('backend session userid='+data.session_userid);
						...
						questions.questions.forEach(function(question) {
							...
							question.choices.forEach(function(choice) {
								...
								choice.votes.forEach(function(vote) {						
									...
									if (vote.userid.toLowerCase === (data.session_userid).toLowerCase()) {
